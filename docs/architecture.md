...
The role-local Gateway management listener remains bound to loopback and authenticated with the local management secret. Its versioned control-plane read model (`GET /v1/operations`) reuses the same operations diagnostics as MCP deployment attestation and the same live `MembershipWorkerRegistry` as Gateway routing. It exposes a redacted Worker projection with liveness plus the Worker-authoritative Workspace catalog (root, profile, tool policy, and command allowlist); Worker credential references are never part of that response.

Versioned control-plane mutations reuse existing authority owners rather than introducing Dashboard state. Worker enrollment, transport update, and removal continue through `EnrollmentService`. Workspace add/remove, profile changes, tool policy, and command allowlist changes are delegated to the selected enrolled Worker through the optional `workspace-admin-v1` Worker capability. The Worker resolves new Workspace roots locally, atomically updates its own runtime config, and hot-reloads the same Workspace catalog used for execution. CLI and Worker mutations share the same `@queqiao/operations` mutation functions and atomic config-store primitive.

The control-plane API is not public MCP and does not grant Workspace authority directly from the Gateway. The Local Operations Dashboard is served only by the loopback management listener. `queqiao dashboard open` authenticates with the local management secret, asks the Gateway to mint a one-time in-memory browser code, and launches the Dashboard with that code in the URL fragment so it is not sent in the initial HTTP request. The Dashboard exchanges the code once for a short-lived in-memory session token carried only in the `x-queqiao-dashboard-session` header. Browser sessions cannot mint replacement sessions, are revocable, and do not receive the master management secret. The existing manual management-secret form remains a local recovery path.

Under the current deployed loopback Worker transport, the enrolled Worker credential authenticates the optional admin capability as part of the trusted Gateway-to-Worker channel. That assumption MUST NOT be carried unchanged into a future non-loopback Worker transport; remote Worker administration requires a stronger or separately scoped authenticated control channel before `workspace-admin-v1` is exposed remotely.
