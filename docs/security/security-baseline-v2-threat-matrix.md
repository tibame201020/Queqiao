...
| Fresh Windows setup inherits overly broad ACL from parent | **Medium pre-CLI blocker** | Not fixed/claimed by v2; explicit Windows ACL hardening and doctor acceptance required before setup release | Read-only audit reproduced parent-inheritance dependence; production ACL metadata currently restricted |
