...
export function loadGatewayConfig(env: NodeJS.ProcessEnv = process.env): GatewayRuntimeConfig {
  const publicBaseUrl = new URL(required(env, "PUBLIC_BASE_URL")); publicBaseUrl.pathname = publicBaseUrl.pathname.replace(/\/$/, "");
...
export function loadGatewayConfigFile(file: string): GatewayRuntimeConfig {
  ...
  const publicBaseUrl = new URL(gateway.publicBaseUrl); publicBaseUrl.pathname = publicBaseUrl.pathname.replace(/\/$/, "");
  if (publicBaseUrl.search || publicBaseUrl.hash) throw new Error("gateway.publicBaseUrl cannot contain query or fragment");
  if (publicBaseUrl.protocol !== "https:" && !["localhost", "127.0.0.1"].includes(publicBaseUrl.hostname)) throw new Error("gateway.publicBaseUrl must use HTTPS except on localhost");
  ...
  if (!workers.length) throw new Error("At least one environment is required");
