...
function secret(env: NodeJS.ProcessEnv, name: string): string {
  ...
  return file ? readFileSync(path.resolve(file), "utf8").trim() : required(env, name);
}
...
function normalizePublicBaseUrl(rawUrl: string, label: string): URL {
  const url = new URL(rawUrl);
  if (url.search || url.hash) throw new Error(`${label} cannot contain query or fragment`);
  if (url.protocol !== "https:" && !["localhost", "127.0.0.1"].includes(url.hostname)) throw new Error(`${label} must use HTTPS except on localhost`);
...
export function loadGatewayConfig(env: NodeJS.ProcessEnv = process.env): GatewayRuntimeConfig {
  ...
  return {
    ...
    stateDir: path.resolve(required(env, "QUEQIAO_STATE_DIR")),
    approvalSecret: secret(env, "OAUTH_APPROVAL_SECRET"),
    jwtSecret: new TextEncoder().encode(signingSecret),
    trustProxyHops: integer(env, "TRUST_PROXY_HOPS", 1),
    allowedRedirectOrigins: new Set((env.OAUTH_ALLOWED_REDIRECT_ORIGINS || "https://chatgpt.com,http://127.0.0.1,http://localhost").split(",").map((item) => new URL(item.trim()).origin)),
    extensions: [],
...
export function loadGatewayConfigFile(file: string): GatewayRuntimeConfig {
  ...
  return {
    ...
    publicBaseUrl,
    resourceUrl: new URL("mcp", publicBaseUrl).href,
    stateDir: path.resolve(gateway.stateDirectory),
    approvalSecret,
    jwtSecret: new TextEncoder().encode(signingSecret),
    trustProxyHops: gateway.trustProxyHops,
