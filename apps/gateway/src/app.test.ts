...
describe("Queqiao v0 vertical slice", () => {
  ...
  beforeEach(async () => {
    ...
    const worker = await createWorkerApp({ workerId: TEST_WORKER_ID, environmentId: "windows", workspaces: [{ id: "fixture", displayName: "Fixture", root: temporary }, { id: "secondary", displayName: "Secondary", root: secondary, profile: "coding", commands: { allow: [path.basename(process.execPath).toLowerCase()] } }], workerToken: TEST_WORKER_CREDENTIAL });
  ...
  it("completes OAuth, lists the stable v0 tools, and reads through the worker", async () => {
    ...
    try {
      ...
      const implicit = await client.callTool({ name: "workspace_info", arguments: {} });
      expect(implicit.isError).toBe(true);
      expect(JSON.stringify(implicit.content)).toContain("workspace_required");
      expect(JSON.stringify((await client.callTool({ name: "workspace_info", arguments: { workspaceId: "fixture" } })).content)).toContain("windows");
      ...
      const search = await client.callTool({ name: "search_text", arguments: { workspaceId: "fixture", query: "native worker", globs: ["**/*.txt"] } });
      expect(search.isError).not.toBe(true);
      expect(JSON.stringify(search.content)).toContain("fixture.txt");
      const extensionList = await client.callTool({ name: "extension", arguments: { workspaceId: "fixture", operation: "list" } });
      ...
      expect(listedPayload.deployment.publicToolCount).toBe(CORE_PUBLIC_TOOLS.length);
