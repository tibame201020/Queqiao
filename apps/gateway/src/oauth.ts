...
function validRedirectUri(value: unknown, allowedOrigins: ReadonlySet<string>): value is string {
  ...
  try {
    ...
    if (url.username || url.password || url.hash || !allowedOrigins.has(url.origin)) return false;
