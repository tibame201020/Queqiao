...
export async function startRuntime(configFile: string, layout: RuntimeLayout, roleValue: string, nameValue: string, dependencies: Dependencies = {}) {
  ...
  if (platform === "win32") {
    const ps = windowsSystemExecutable("WindowsPowerShell\\v1.0\\powershell.exe", env); const q = (v: string) => `'${v.replaceAll("'", "''")}'`; const command = `$env:QUEQIAO_CONFIG_FILE=${q(path.resolve(configFile))}; $env:QUEQIAO_AUDIT_DIR=${q(path.join(layout.stateDir, "audit"))}; $p=Start-Process -FilePath ${q(nodePath)} -ArgumentList @(${q(entryPoint)}) -WorkingDirectory ${q(path.dirname(entryPoint))} -WindowStyle Hidden -PassThru; [Console]::Out.Write($p.Id)`; pid = Number((await execFile(ps, ["-NoLogo", "-NoProfile", "-NonInteractive", "-Command", command])).stdout.trim());
