...
describe("repo runtime migration", () => {
  it("separates config, secrets, state, and repository-local workspace data", async () => {
    ...
    const layout = resolveRuntimeLayout(process.platform === "win32" ? { LOCALAPPDATA: path.join(root, "local"), TEMP: path.join(root, "run") } : { HOME: root, XDG_CONFIG_HOME: path.join(root, "cfg"), XDG_DATA_HOME: path.join(root, "data"), XDG_STATE_HOME: path.join(root, "state"), XDG_RUNTIME_DIR: path.join(root, "run") }, process.platform); await migrateFromRepository(repo, layout, true);
  ...
  it("upgrades external runtime v1 files into one YAML document", async () => {
    temporary = await mkdtemp(path.join(os.tmpdir(), "queqiao-v1-")); const layout = resolveRuntimeLayout(process.platform === "win32" ? { LOCALAPPDATA: path.join(temporary, "local"), TEMP: path.join(temporary, "run") } : { HOME: temporary, XDG_CONFIG_HOME: path.join(temporary, "cfg"), XDG_DATA_HOME: path.join(temporary, "data"), XDG_STATE_HOME: path.join(temporary, "state") }, process.platform);
