...
export async function createWorkerApp(config: WorkerAppConfig): Promise<Express> {
  ...
  app.post("/v1/tools/:toolName", async (req, res) => {
    if (!tools.definitions().some(({ name }) => name === req.params.toolName)) return res.status(404).json({ error: "tool_not_found" });
    ...
    try { res.json({ result: await tools.execute(req.params.toolName, req.body, { catalog, processes, signal: abort.signal }) }); }
    catch (error) {
  ...
  app.post("/v1/read-file", async (req, res) => {
    const parsed = readRequestSchema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: "invalid_request" });
    try { res.json(await tools.execute("read_file", parsed.data, { catalog, processes })); }
    catch (error) {
      if (error instanceof WorkerToolError) return res.status(error.status).json({ error: error.code, message: error.message });
