...
export function resolveRuntimeLayout(env: NodeJS.ProcessEnv = process.env, platform: NodeJS.Platform = process.platform): RuntimeLayout {
  ...
  const windows = platform === "win32"; const home = (windows ? env.USERPROFILE || env.HOME : env.HOME || env.USERPROFILE) || os.homedir(); const localAppData = env.LOCALAPPDATA || paths.join(home, "AppData", "Local");
