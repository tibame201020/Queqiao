...
import { requireRuntimeConfigFile, resolveExtensionHubRoot, resolveNamedRoleConfigRoot, resolveRuntimeLayout, resolveRuntimeLayoutForInstance, resolveRuntimeLayoutForNamedRole } from "./index.js";
describe("Runtime Layout v1", () => {
  ...
  it("gives the Extension Hub its own global data root without global config or state ownership", () => {
    expect(resolveExtensionHubRoot({ LOCALAPPDATA: "C:\\Users\\owner\\AppData\\Local" }, "win32")).toBe("C:\\Users\\owner\\AppData\\Local\\Queqiao\\data\\extensions");
    expect(resolveExtensionHubRoot({ HOME: "/home/owner", XDG_DATA_HOME: "/data" }, "linux")).toBe("/data/queqiao/extensions");
  });
  it("requires an explicit runtime config file for executable runtime entry points", () => {
    expect(requireRuntimeConfigFile({ QUEQIAO_CONFIG_FILE: "C:\\runtime\\worker.yaml" }, "win32")).toBe("C:\\runtime\\worker.yaml");
    expect(() => requireRuntimeConfigFile({}, "win32")).toThrow(/QUEQIAO_CONFIG_FILE is required/);
  });
