...
describe("extension config schema", () => {
  ...
  it("keeps the verified Gateway listener loopback-only and liveness polling low-frequency", () => {
    ...
    expect(parsed.gateway?.livenessIntervalMs).toBe(30_000);
    expect(runtimeConfigSchema.parse({ ...base, gateway: { ...gateway, livenessIntervalMs: 5_000 } }).gateway?.livenessIntervalMs).toBe(5_000);
