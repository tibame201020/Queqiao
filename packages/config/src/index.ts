...
export class ConfigLockedError extends Error {
  constructor(file: string) {
    super(`Configuration is locked by another Queqiao process: ${file}`);
    this.name = "ConfigLockedError";
  }
}

export class AtomicConfigStore<T extends RuntimeConfig = RuntimeConfig> {
  constructor(readonly file: string, private readonly validate: (value: unknown) => T = (value) => runtimeConfigSchema.parse(value) as T) {}

  async read(): Promise<T> {
    return this.validate(parse(await readFile(this.file, "utf8")));
  }
  ...
  async initialize(value: T): Promise<T> {
    ...
    try { await handle.writeFile(serializeRuntimeConfig(validated), "utf8"); }
  ...
  async update(mutator: (current: T) => T): Promise<T> {
    ...
    try { lock = await open(lockFile, "wx", 0o600); }
      ...
      if ((error as NodeJS.ErrnoException).code === "EEXIST") throw new ConfigLockedError(this.file);
      throw error;
    }
    try {
      const next = this.validate(mutator(await this.read()));
      const temporary = `${this.file}.${process.pid}.${Date.now()}.tmp`;
      await writeFile(temporary, serializeRuntimeConfig(next), { encoding: "utf8", mode: 0o600 });
  ...
  async metadata() {
    ...
    return { file: this.file, bytes: info.size, modifiedAt: info.mtime.toISOString() };
  }
}
